...
## Choose whether to allow multi-jump (double jump, triple jump, etc.)
export var allow_multi_jump := false
...
func physics_movement(delta: float, player_body: PlayerBody):
	# Skip if the player isn't on the ground and multi-jump is off, otherwise allow multi-jump
	if !player_body.on_ground and allow_multi_jump == false:
		return
	...
	# Skip if the jump button isn't pressed and reset jump counter
	...
	#Skip if still holding original jump button to avoid flying when using multi-jump
	if jump_count > 0:
		return
