...
func _on_GhostArea_body_entered(body):
	...
	if body == player_body:
		...
		if escape == true:
			...
			self.global_transform.origin = ghost_starting_position # Replace with function body.
...
func escape_mode():
	escape = true
	$Ghost/Icosphere001.set_surface_material(0, escape_material) #code to switch material
